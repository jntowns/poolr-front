<template>
  <div class="max-w-2xl mx-auto my-8 px-4">
    ...
    <transition v-else name="fade" mode="out-in">
      ...
      >
        <h2 class="text-2xl font-semibold mb-4 text-gray-800">
          {{ t("offer_verifyHeader") }}
        </h2>
        <p class="mb-6 text-gray-600">
          {{ t("offer_verifyMessage") }}
        </p>
        ...
        <form @submit.prevent="handleUpload">
          ...
          <div class="relative my-6 text-gray-500 font-semibold">
            ...
            <div class="relative flex justify-center">
              <span class="bg-white px-4">{{ t("or") }}</span>
            </div>
          </div>
          ...
          <div class="mb-6">
            ...
            >
              <span v-if="!selectedFile && !previewUrl">{{
                t("offer_uploadPlaceholder")
              }}</span>
              <span v-else-if="selectedFile && !isDemoLicense">{{
                selectedFile.name
              }}</span>
              <span v-else>{{ t("offer_demoSelected") }}</span>
            </label>
          </div>
      ...
      >
        ...
        ></div>
        <h2 class="text-2xl font-semibold mb-4 text-gray-800">
          {{ t("offer_verifyingHeader") }}
        </h2>
        <p class="text-gray-600">
          {{ t("offer_verifyingMessage") }}
        </p>
      </div>
      ...
      >
        <h2 class="text-3xl font-semibold mb-4 text-green-600">
          ✓ {{ t("offer_verifiedDriver") }}
        </h2>
        <p class="text-gray-600">{{ t("offer_verifiedMessage") }}</p>
      </div>
    </transition>
  </div>
</template>
...
const useDemoLicense = () => {
  ...
  showToast(t("toast_demoLicenseSelected"), "info");
};
...
const handleUpload = async () => {
  ...
  setTimeout(() => {
    ...
    showToast(t("toast_licenseVerified"), "success");
